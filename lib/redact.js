...
Redact.elementQuery = function RedactElementQuery (element, key, value) {
  return _.object(
    [ Redact.mongoKey(Redact.extractKey(element), key) ],
    [ value ]
  )
}
...
Redact.unlockElement = function RedactUnlockElement (collection, element) {
  return new Promise(function (resolve, reject) {
    collection.update(
      document,
      {
        $unset: Redact.elementQuery(element, '_lock', '')
      },
      function (err, data) {
        err
          ? reject(err)
    ...
    console.log(element.document,
      {
        $set: _.object(
          [
            Redact.mongoKey(Redact.extractKey(element), key),
            Redact.mongoKey(Redact.extractKey(element), '_lock', '_time')
          ],
...
Redact.updateAndUnlockElement = function RedactUpdateAndUnlockElement (collection, element, key, value) {
  return Redact.updateFieldValue
    ...
    .then(Redact.lockElement.bind(null, arguments))
}
